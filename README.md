...
初めて触る方向けに、サインアップからタスク管理までの一連の流れをまとめます。
...
プロジェクトを作成した直後は、オーナー本人しかそのプロジェクトのメンバーになっていません。同じチームの他メンバーにタスクを見せたり担当させたりするには、プロジェクト詳細画面のメンバー欄から「同じチームに所属するユーザー」を明示的に追加する必要があります。ここを行わないと、チームメンバーが「タスクが見えない」状態になるので注意してください。
